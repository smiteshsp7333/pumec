...
export default function ServicesClient({ services }: { services: any[] }) {
  // If backend fails or returns empty, use a fallback to prevent blank pages
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            ...
            <p className="text-gray-300 text-lg md:text-[19px] leading-relaxed mb-0 max-w-2xl font-medium">
              From startup compliance to multinational tax strategy — we provide the rigorous architecture your business needs. All managed from the dashboard.
            </p>
          </motion.div>
        </div>
      ...
      <section className="py-24 relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {displayServices.length === 0 ? (
            <div className="text-center text-gray-500 py-12">No services found. Add them in the Admin Panel.</div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {displayServices.map((service, i) => {
                const Icon = getIcon(service.icon || 'Box');
                return (
                  <motion.div 
                    ...
                    <div className="relative z-10 flex-1 flex flex-col">
                      ...
                      {service.challenges && service.challenges.length > 0 && (
                        <ul className="space-y-3 mb-8">
                          {service.challenges.map((detail: string, idx: number) => (
