...
export default function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const post = {
    ...
    content: `
      ...
      <h2 className="text-2xl font-bold text-navy mt-12 mb-6">Key Policy Shifts</h2>
      ...
      <h3 className="text-xl font-bold text-navy mt-8 mb-4">Impact on Manufacturing</h3>
      ...
      <div className="bg-bg-gray border-l-4 border-accent-blue p-8 my-10 rounded-r-2xl">
        <p className="italic text-navy text-lg">"The relaxation of FDI norms in the space sector is not just about capital; it's about technology transfer and putting India on the global aerospace map."</p>
      </div>
      
      <h2 className="text-2xl font-bold text-navy mt-12 mb-6">Compliance Checklist for Investors</h2>
      <ul className="list-disc pl-6 space-y-4 mb-8 text-text-muted">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-12">
            ...
            >
              ...
              <h1 className="text-3xl lg:text-6xl font-bold text-navy leading-tight tracking-tight mb-8">
                {post.title}
              </h1>
              
              <div className="flex flex-wrap items-center justify-between gap-8 py-8 border-y border-border-subtle">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full overflow-hidden bg-bg-gray border border-border-subtle">
                    <Image src={post.author.image} alt={post.author.name} width={48} height={48} />
                  </div>
                  <div>
                    <h4 className="text-navy font-bold text-sm tracking-wide">{post.author.name}</h4>
                    <p className="text-text-muted text-xs font-medium uppercase tracking-wider">{post.author.role}</p>
                  </div>
                </div>
          ...
          >
             <div className="text-text-muted leading-relaxed" dangerouslySetInnerHTML={{ __html: post.content }} />
          </motion.div>

          {/* Footer / CTA */}
          <div className="mt-20 pt-12 border-t border-border-subtle">
             <div className="bg-bg-gray rounded-3xl p-10 lg:p-16 text-center">
                <h3 className="text-2xl lg:text-3xl font-bold text-navy mb-6">Need expert advice on FDI or Corporate Law?</h3>
                <p className="text-text-muted mb-10 max-w-xl mx-auto font-medium">Schedule a confidential consultation with CA Mehra and our regulatory experts today.</p>
